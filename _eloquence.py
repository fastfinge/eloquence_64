...
class AudioWorker(threading.Thread):
    ...
    def __init__(self, player: nvwave.WavePlayer, audio_queue: "queue.Queue[Optional[AudioChunk]]"):
        ...
        self._final_waiter: Optional[threading.Thread] = None
        self._bytes_per_second = self._CHANNELS * (self._BITS_PER_SAMPLE // 8) * self._SAMPLE_RATE
        ...
        while self._running:
            ...
            while tries < 10:
                try:
                    ...
                    expected_finish = self._register_playback(len(data))
                    if tries:
                        LOGGER.warning("Audio feed retries=%d", tries)
                    if is_final:
                        self._schedule_final(expected_finish)
                    fed = True
                    break
                except Exception:
    ...
    def _register_playback(self, byte_length: int) -> float:
        duration = byte_length / float(self._bytes_per_second)
        now = time.perf_counter()
        with self._state_lock:
            start = max(now, self._playback_deadline)
            finish = start + duration
            self._playback_deadline = finish
        return finish

    def _schedule_final(self, expected_finish: Optional[float] = None) -> None:
        with self._state_lock:
            deadline = expected_finish or self._playback_deadline or time.perf_counter()
            if self._final_pending:
                if deadline > self._final_deadline:
                    self._final_deadline = deadline
                return
            self._final_pending = True
            self._final_deadline = deadline
            waiter = threading.Thread(target=self._await_final, name="EloquenceFinalizer", daemon=True)
            self._final_waiter = waiter
        waiter.start()

    def _await_final(self) -> None:
        while True:
            with self._state_lock:
                deadline = self._final_deadline
                pending = self._final_pending
            if not pending:
                return
